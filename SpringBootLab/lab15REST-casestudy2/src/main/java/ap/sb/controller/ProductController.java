...
public class ProductController {
	...
	public ResponseEntity<String> a1GetMessage() {
		...
		return new ResponseEntity<String>(productService.getMessage(), HttpStatus.OK);
	...
	@GetMapping("/read/{id}")
	public ResponseEntity<?> a3FindById(@PathVariable int id) {
	...
	public ResponseEntity<?> a5Update(@PathVariable int id, @Valid @RequestBody Product product) {
		...
		if (foundProduct!=null ) {
			Product revisedProduct = foundProduct;
			response = ResponseEntity.ok(revisedProduct);
		}
